...
module.exports = async function grandmasterLeague(region) {
  const rAPI = await new RiotAPI(process.env.RIOT_API_KEY);

  const grandmaster = await rAPI.tftLeague.getGrandmaster({
    region,
  });

  grandmaster.entries.map(async (entry) => {
    try {
      await db.query(
  ...
  console.log(region, "grandmaster done");
};
