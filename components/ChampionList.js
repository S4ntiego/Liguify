...
import ChampionsList from '../apis/ChampionsList'
import { ChampionsContext } from '../context/ChampionsContext'

export function ChampionList() {
  const { data, setData, setActiveTab } = useContext(ChampionsContext)
  ...
  const columns = useMemo(
    () => [
      columnHelper.accessor('champion_name', {
        header: () => <div className="pl-4 text-left">Champion</div>,
        cell: (info) => (
          <div className="flex items-center pl-4">
            <div className="relative h-10 w-10">
              <Image
                ...
                layout="fill"
                objectFit="cover"
                src={`https://cdn.tft.tools/face/tft7_${info.row.original.champion_api_name}.jpg`}
              />
            </div>
            <div className="pl-4">
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      if (!data) {
        try {
          const response = await ChampionsList.get()

          setData(response.data.data.champions)
        } catch (err) {
          console.log(err)
        }
      }
              ...
              {table.getRowModel().rows.map((row) => (
                ...
                >
                  {row.getVisibleCells().map((cell) => (
                    <Link
                      href={`/champions/${
                        cell.getContext().row.original.champion_name
                      }`}
                    >
                      <td
