...
const Champion = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const response = await ChampionsList.get(`${name}`)

        setSelectedChampion(response.data.data.championData)
      } catch (err) {
        console.log(err)
      }
    }
  ...
  const columns = React.useMemo(
    () => [
      columnHelper.accessor('three_items', {
        header: () => <div className="pl-4 text-left">Build</div>,
        cell: (info) => (
            ...
            {info.getValue().map((i) => (
              <div className="relative h-10 w-10">
                <Image
                  ...
                  layout="fill"
                  objectFit="cover"
                  src={`https://cdn.tft.tools/items/70_${i}.jpg`}
                />
              </div>
            ))}
          </div>
      ...
      columnHelper.accessor('percentage_frequency', {
        ...
        header: () => <div className="pl-10">Play Ratio</div>,
      }),
